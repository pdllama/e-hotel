...
export async function load({ locals, params, url }:any) {
    ...
    const works_data = await dbPool.query(`SELECT * FROM works_in NATURAL JOIN person NATURAL JOIN employee WHERE address_id = '${params.hotel_id}' AND SSN = ${params.SSN}`).then(r => r.rows[0])
