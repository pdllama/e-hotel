<script lang="ts">
    ...
    const state = $state({
        phone_numbers: [...data.hotel.phone_numbers],
        emails: [...data.hotel.emails]
    })

    const finalizeEdits = async() => {
        const res = await fetch(`/hotel/${data.hotel_id}/manage/hotel/edit`, {
            method: 'PUT',
            body: JSON.stringify({hotel_id: data.hotel_id, phone_numbers: state.phone_numbers, emails: state.emails, oldPhoneNumbers: data.hotel.phone_numbers, oldEmails: data.hotel.emails})
        }).then(r => r.json())

        if (res.success) {
            addNotification({body: 'Changed Contact info!', success:true, errorStatus: 201})
