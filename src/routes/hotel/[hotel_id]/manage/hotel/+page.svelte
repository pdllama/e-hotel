...
<div class='flex flex-col justify-start ml-3'>
    ...
    <div class="w-[300px] min-w-[300px] flex flex-col mt-5">
        ...
        {#each data.hotel.emails.slice(1, data.hotel.emails.length) as extra_email}
        ...
        {#each data.hotel.phone_numbers.slice(1, data.hotel.phone_numbers.length) as extra_number}
            <p class='text-[12px] text-end'>{extra_number}</p>
        {/each}
    </div>
    {#if data.user.is_manager}
    <Button buttonClasses='p-2 bg-indigo-200 rounded-lg mt-5 w-[150px] hover:bg-indigo-300' onClick={() => goto(`/hotel/${data.hotel_id}/manage/hotel/edit`)}>Edit Contact Information</Button>
