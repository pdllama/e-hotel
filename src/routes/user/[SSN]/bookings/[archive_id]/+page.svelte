<script lang="ts">
    ...
    const cancelBooking = async() => {
        await fetch(`/user/${data.user.SSN}/bookings/${data.archive.archive_id}`, {method: 'PUT'})
        location.reload()
    }
...
<div class='flex flex-col justify-start ml-3'>
    ...
    <p class='text-[18px] mt-5'><span class='font-bold'>Hotel: </span> {data.archive.chain_name}</p>
    <p class='text-[18px]'><span class='font-bold'>Address: </span> {data.archive.street_number} {data.archive.street_name} | {data.archive.city}, {data.archive.state}, {data.archive.country}</p>
    ...
    {/if}
    {#if (data.archive.status == 'booked')}
    <Button buttonClasses='p-2 bg-indigo-200 rounded-lg mt-5 w-[150px] hover:bg-indigo-300 absolute bottom-0' onClick={() => cancelBooking()}>Cancel Booking</Button>
    {/if}
</div>