...
>
    ...
    <div class='flex flex-col justify-start items-start w-[100%]'>
        <p class='text-[24px] font-bold'>{`Room ${problem_data.room_number}`}</p>
        <p class='text-[14px] font-bold'>{problem_data.type[0].toUpperCase()}{problem_data.type.slice(1, problem_data.type.length)} Problem{list_status ? ` ${problem_data.status[0]}${problem_data.status.slice(1, problem_data.status.length)}` : ''}</p> 
        <p class='text-[14px]'>
